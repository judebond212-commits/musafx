...
import { HiUsers, HiSearch, HiEye, HiX } from 'react-icons/hi'
...
function UserModal({ user, onClose }) {
  ...
  const fields = [
    ...
    ['Investment Plan', user.investmentPlan || '—'], ['Investment Amount', user.investmentAmount ? `$${user.investmentAmount.toLocaleString()}` : '$0'],
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '28px' }}>
        ...
        <p style={{ color: '#555', fontSize: '14px' }}>Manage all registered user accounts.</p>
      </div>
              ...
              <tbody>
                {filtered.map((user, i) => (
                  <tr key={user.userID} style={{ borderBottom: i < filtered.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                    <td style={{ padding: '13px 14px', color: '#444', fontSize: '12px' }}>{user.userID}</td>
                    ...
                    <td style={{ padding: '13px 14px', fontWeight: '600' }}>${(user.investmentAmount || 0).toLocaleString()}</td>
